...
class AccessControlMixin:
    ...
    def __getattribute__(self, name):
        ...
        # 2) Luego: control basado en prefijos de nombre
        #    para atributos y métodos no decorados
        ...
        if self._debug_mode:
            ...
            if level == "private":
                raise PrivateAccessError(
                    f"Access to private member '{name}' is forbidden."
                )
            elif level == "protected":
                raise ProtectedAccessError(
                    f"Access to protected member '{name}' is forbidden."
                )
